...
#pragma mark - NavigationBar
    ...
    QMUICMI.navBarShadowImage = [UIImage qmui_imageWithColor:self.themeTintColor];   // NavBarShadowImage : UINavigationBar.shadowImage，也即导航栏底部那条分隔线
...
#pragma mark - Others
    ...
    QMUICMI.automaticCustomNavigationBarTransitionStyle = YES;
...
- (BOOL)shouldApplyTemplateAutomatically {
    NSObject *obj = [[NSUserDefaults standardUserDefaults] objectForKey:@"com.kCurrentTheme.cn"];
    Class currentThemeClass = obj.class;
    if ([NSStringFromClass(currentThemeClass) isEqualToString:NSStringFromClass(self.class)]) {
        [CJThemeManager shareThemeManager].currentTheme = self;
        return YES;
    }
    return NO;
}
